    ...
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-6 py-12">
      ...
      <form onSubmit={handleLogin} className="w-full max-w-sm space-y-4">
        ...
        <button type="submit" disabled={loading}
          className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white font-semibold rounded-xl py-4 flex items-center justify-center gap-2 transition-colors mt-2 text-base">
          {loading ? <><Loader size={18} className="animate-spin" /> Signing in...</> : 'Sign In'}
        </button>

        <p className="text-center text-white/30 text-sm mt-4">Locked out? Contact your manager.</p>
      </form>
      ...
      <div className="mt-8 bg-surface-card border border-white/5 rounded-xl px-4 py-3 w-full max-w-sm">
        ...
        <p className="text-white/40 text-xs text-center">marcus.w@secureedge.co.uk / guard123</p>
      </div>
    </div>
  )
}
