...
export function getVisibleFeatures(role: RoleType): string[] {
  return Object.keys(FEATURE_PERMISSIONS).filter(feature =>
    Object.keys(FEATURE_PERMISSIONS[feature]).some(action =>
      canPerform(role, feature, action)
    )
  )
}

// Route-level nav access — maps frontend paths to which roles can see them
